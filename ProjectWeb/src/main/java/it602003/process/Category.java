...
public class Category {
		...
		public boolean editCategory(CategoryObject category) {
			StringBuilder sql = new StringBuilder();
			sql.append(
					"UPDATE tblcategory SET category_name = ?"
					+ " WHERE category_id = ? ");
		...
		public boolean capNhat(int id) {
			StringBuilder sql = new StringBuilder();
			sql.append("UPDATE tblcategory SET category_total_book = ( SELECT COUNT(book_inventory_number) FROM tblbook WHERE tblbook.category_id = tblcategory.category_id ) WHERE category_id = ? ;");
			try {
				PreparedStatement pre = this.con.prepareStatement(sql.toString());
				pre.setInt(1, id);
				int result = pre.executeUpdate(); // return ra rowDeleted (row đã delete)
